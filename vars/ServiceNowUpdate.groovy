...
def call() {
    withCredentials([usernamePassword(credentialsId: ServiceNow, passwordVariable: 'PASSWORD', usernameVariable: 'USERNAME')]) {
        ...
        // Get sys_id of the request
        def curlResponse = sh(returnStdout: true, script: """
            curl -s -k -u ${USERNAME}:'${PASSWORD}' \
            "${SNApi}/table/${table_type}?sysparm_query=number=${REQUEST_NUMBER}&sysparm_fields=sys_id"
        """)
        def json = readJSON(text: curlResponse)
        def sn_request_sys_id = json.result[0].sys_id

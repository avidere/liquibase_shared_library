def call(def ServiceNow, def REQUEST_NUMBER, def SNApi) {
    ...
    try {
       ...
       // withCredentials([usernamePassword(credentialsId: ServiceNow, passwordVariable: 'PASSWORD', usernameVariable: 'USERNAME')]) {
            def curlResponse = sh(returnStdout: true, script: "curl -s -k -u admin:A@vinash2412 'https://dev313863.service-now.com/api/now/table/sc_request?sysparam_display_value=true&sysparam_query=number=number=REQ0010001' | jq -r .result[]")
